...
const ITD=LEVEL.meta.perspective!=='platformer';
...
class G extends Phaser.Scene{
  ...
  create(){
    ...
    var ht=!!(ASSETS.tilesetUrl&&this.textures.exists('ts'));
    var TF=256;
    LEVEL.rooms.forEach(function(room){
      ...
      room.floorTiles.forEach(function(row,ri){
        row.forEach(function(cell,ci){
          ...
          if(cell===1){
            if(ht){
              this.add.image(wx+T/2,wy+T/2,'ts').setCrop(0,0,TF,TF).setDisplaySize(T,T).setDepth(1);
            } else {
              this.gF.fillStyle(hx(LEVEL.palette.floor),1);
              this.gF.fillRect(wx,wy,T,T);
              this.gF.fillStyle(0xffffff,0.05);
              this.gF.fillRect(wx,wy,T,2);
            }
            if(!ITD){
              this.gW.fillStyle(hx(LEVEL.palette.platform||LEVEL.palette.floor),1);
              this.gW.fillRect(wx,wy,T,4);
              this.sb.push({x:wx,y:wy,w:T,h:T,p:true});
            }
          } else if(ITD){
            if(ht){
              this.add.image(wx+T/2,wy+T/2,'ts').setCrop(TF,0,TF,TF).setDisplaySize(T,T).setAlpha(0.9).setDepth(2);
            } else {
              this.gW.fillStyle(hx(LEVEL.palette.wall),1);
              this.gW.fillRect(wx,wy,T,T);
              this.gW.fillStyle(0xffffff,0.04);
              this.gW.fillRect(wx+1,wy+1,T-2,4);
            }
            this.sb.push({x:wx,y:wy,w:T,h:T,p:false});
          }
        },this);
      },this);
      this.add.text(rx+room.w*T/2,ry+6,room.label||room.id,{fontSize:'9px',fontFamily:'monospace',color:'#ffffff'}).setAlpha(0.2).setOrigin(0.5,0).setDepth(3);
      (room.items||[]).forEach(function(item){
        var wp=tw(room.x+item.x,room.y+item.y);
        this.it.push(Object.assign({},item,{wx:wp.x,wy:wp.y,room:room.id,collected:false,bobOffset:Math.random()*Math.PI*2}));
      },this);
      (room.enemies||[]).forEach(function(e){
        var wp=tw(room.x+e.x,room.y+e.y);
        var pt=(e.patrol||[]).map(function(p){return tw(room.x+p.x,room.y+p.y);});
        this.en.push(Object.assign({},e,{wx:wp.x,wy:wp.y,vx:0,vy:0,hp:e.hp||60,mhp:e.hp||60,patrol:pt,pi:0,room:room.id,alive:true,ifc:0,acd:0,bt:Math.random()*6.28,ag:e.type==='boss'?220:130}));
      },this);
    },this);
    ...
    // Camera — use startFollow for correct first-frame positioning
    this.camTarget=this.add.rectangle(sp.x,sp.y,2,2,0x000000,0).setDepth(-1);
    ...
    this.acd=0;
    this.mc=document.getElementById('mm').getContext('2d');
  }
  ...
  update(time,delta){
    ...
    this.it.forEach(function(item){
      ...
      if(Math.hypot(P.x-item.wx,P.y-item.wy)<22){
        item.collected=true;
        if(item.type==='health')this.php=Math.min(this.mhp,(this.php||100)+35);
    ...
    this.en.forEach(function(e){
      ...
      } else {e.vx=(e.vx||0)*0.8;e.vy=(e.vy||0)*0.8;}
    ...
    this.rndr(dt);this.updUI();this.drawMM();
  }
  ...
  doAtk(){
    ...
    var d=dirs[P.facing]||[P.dir,0];
    var ax=P.x+d[0]*38,ay=P.y+d[1]*38;
    ...
    this.en.forEach(function(e){
      ...
      if(Math.hypot(e.wx-ax,e.wy-ay)<32){
        ...
        if(e.hp<=0){e.alive=false;self.burst(e.wx,e.wy,e.color||'#ff4466',18);}
      }
    });
  }
  ...
  burst(x,y,ch,n){
    ...
    for(var i=0;i<n;i++){
      var a=Math.random()*Math.PI*2,v=50+Math.random()*100;
  ...
  rndr(dt){
    ...
    this.it.forEach(function(item){
      ...
      if(item.type==='key'){
        ...
        this.gI.fillRect(item.wx+6,item.wy+bob+1,3,3);
      } else if(item.type==='health'){
        this.gI.fillStyle(0xff4466,0.9);
        this.gI.fillCircle(item.wx-3,item.wy+bob-2,5);
        this.gI.fillCircle(item.wx+3,item.wy+bob-2,5);
        this.gI.fillTriangle(item.wx-7,item.wy+bob,item.wx+7,item.wy+bob,item.wx,item.wy+bob+8);
      } else {
        this.gI.fillRect(item.wx-7,item.wy+bob-5,14,14);
      }
      this.gI.fillStyle(c,0.12);
      this.gI.fillCircle(item.wx,item.wy+bob,16);
    },this);
    ...
    this.en.forEach(function(e){
      ...
      if(e.type!=='boss'){
        for(var i=0;i<3;i++){this.gE.fillTriangle(e.wx-s/2+i*s/3,e.wy+bob-s/2,e.wx-s/2+i*s/3+s/6,e.wy+bob-s/2-9,e.wx-s/2+(i+1)*s/3,e.wy+bob-s/2);}
      } else {
        this.gE.lineStyle(2,0xff4466,0.4+Math.sin(t*4)*0.4);
        this.gE.strokeRect(e.wx-s/2-5,e.wy-s/2-5,s+10,s+10);
      }
      this.gE.fillStyle(0xffffff,1);
      this.gE.fillRect(e.wx-s/2+3,e.wy+bob-s/2+4,5,5);
      this.gE.fillRect(e.wx+2,e.wy+bob-s/2+4,5,5);
      this.gE.fillStyle(0xcc0000,1);
      this.gE.fillRect(e.wx-s/2+5,e.wy+bob-s/2+6,2,2);
      this.gE.fillRect(e.wx+4,e.wy+bob-s/2+6,2,2);
      var pct=e.hp/e.mhp;
      this.gE.fillStyle(0x222222,0.8);
      this.gE.fillRect(e.wx-s/2,e.wy+bob-s/2-9,s,4);
      this.gE.fillStyle(pct>0.5?0x44ff88:pct>0.25?0xffaa00:0xff4444,1);
      this.gE.fillRect(e.wx-s/2,e.wy+bob-s/2-9,s*pct,4);
    },this);
    ...
    this.gP.fillEllipse(P.x,P.y+P.h/2+3,P.w,6);
    ...
    this.gP.fillRect(P.x-P.w/2+2,P.y-P.h/2-11,P.w-4,13);
    ...
    if(this.acd>0.18){
      var dirs={right:[1,0],left:[-1,0],up:[0,-1],down:[0,1]};
      var d=dirs[P.facing]||[P.dir,0];
      this.gP.lineStyle(3,0xffffff,this.acd/0.32);
      this.gP.beginPath();
      this.gP.arc(P.x,P.y,32,Math.atan2(d[1],d[0])-0.65,Math.atan2(d[1],d[0])+0.65);
      this.gP.strokePath();
    }
    ...
    this.pts.forEach(function(p){
      ...
      else{this.gX.fillCircle(p.x,p.y,Math.max(0.5,p.r*p.life));}
    },this);
  }
  ...
  updUI(){
    ...
    if(hf){var pct=Math.max(0,(this.php||0)/this.mhp);hf.style.width=(pct*100)+'%';hf.style.background=pct>0.5?'#44ff88':pct>0.25?'#ffaa00':'#ff4444';}
    if(kl&&this.kys&&this.kys.length>0)kl.textContent='Key: '+this.kys.join(', ');
  }

  drawMM(){
    var ctx=this.mc,cw=140,ch=100;
    var wW=LEVEL.meta.width||2560,wH=LEVEL.meta.height||1920;
    var sx=cw/wW,sy=ch/wH;
    ctx.clearRect(0,0,cw,ch);
    var self=this;
    LEVEL.rooms.forEach(function(room){
      ctx.fillStyle=room.id===self.cr?'#3a5a3a':'#2a2a3a';
      ctx.fillRect(room.x*T*sx,room.y*T*sy,room.w*T*sx,room.h*T*sy);
      ctx.strokeStyle='#444';ctx.lineWidth=0.5;
      ctx.strokeRect(room.x*T*sx,room.y*T*sy,room.w*T*sx,room.h*T*sy);
      self.en.filter(function(e){return e.room===room.id&&e.alive;}).forEach(function(e){
        ctx.fillStyle='#ff4466';ctx.fillRect(e.wx*sx-1.5,e.wy*sy-1.5,3,3);
      });
    });
    ctx.fillStyle='#00d4ff';
    ctx.beginPath();ctx.arc((this.pl.x||0)*sx,(this.pl.y||0)*sy,3,0,Math.PI*2);ctx.fill();
  }

  sm(text,dur){
    var el=document.getElementById('msg');if(!el)return;
...
`
  ...
  return [
    ...
    '<style>', css, '</style>',
    '</head><body>',
    '<div id="ui"><div>HP: <span id="hv">100</span></div>',
    '<div id="hb"><div id="hf" style="width:100%"></div></div>',
    '<div id="rl"></div><div id="kl"></div></div>',
    '<div id="as">AI art active</div>',
    '<div id="msg"></div>',
    '<canvas id="mm" width="140" height="100"></canvas>',
    '<div id="ctrl">WASD / Arrows move | Z attack | R restart</div>',
    '<script src="https://cdnjs.cloudflare.com/ajax/libs/phaser/3.60.0/phaser.min.js"></scr' + 'ipt>',
    '<script>', gameScript, '</scr' + 'ipt>',
    '</body></html>',
  ].join('')
}
